...
static void
s_handle_io_error(const char* reason)
{
#   ifdef __WINDOWS__
...
#   endif
        ...
        if (errno == EPIPE
            ...
            fmt::print("E: (udp) error {} on {}", strerror(errno), reason);
            //assert(false);
        }
}
...
udp_new(int port_nbr)
{
    ...
    if (setsockopt(self->handle, SOL_SOCKET,
        SO_BROADCAST, (const char*)&on, sizeof(on)) == -1)
    ...
    soaddr.sin_port = htons(self->port_nbr);
    soaddr.sin_addr.s_addr = htonl(INADDR_ANY);
    if (bind(self->handle, (sockaddr*)&soaddr, sizeof(soaddr)) == -1)
...
{
    assert(self);
    inet_pton(AF_INET, "255.255.255.255", &self->broadcast.sin_addr);
    //inet_aton("255.255.255.255", &self->broadcast.sin_addr);

    if (sendto(self->handle, (const char*) buffer, length, 0,
        (const sockaddr*)&self->broadcast, sizeof(struct sockaddr_in)) == -1)
...
udp_recv(udp_t* self, byte* buffer, size_t length)
{
    ...
    size_t size = recvfrom(self->handle, (char*)buffer, length, 0, (sockaddr*)&soaddr, &si_len);
    if (size == -1)
